...
# Local receipt file path
LOCAL_REPO_PATH = os.path.expanduser("~/receipts")
RECEIPT_FILE_NAME = "receipt.txt"
RECEIPT_FILE_PATH = os.path.join(LOCAL_REPO_PATH, RECEIPT_FILE_NAME)

# Laptop details for SCP transfer
LAPTOP_IP = "192.168.1.103"  # Replace with your laptop's IP address
LAPTOP_USER = "ngk"  # Replace with your laptop's username
DESTINATION_PATH = "/home/ngk/microchip/receipts/"  # Replace with desired destination on the laptop
...
def save_receipt_to_file(item_name, weight, price):
    """
    Save the receipt to the local file and transfer it to the laptop using SCP.
    """
    # Format the receipt
    receipt_text = (
        "---- Receipt ----\n"
    ...
    print(f"Receipt saved to {RECEIPT_FILE_PATH}")

    # Transfer the receipt to the laptop using SCP
    scp_command = f"scp {RECEIPT_FILE_PATH} {LAPTOP_USER}@{LAPTOP_IP}:{DESTINATION_PATH}"
    try:
        subprocess.run(scp_command, shell=True, check=True)
        print("Receipt successfully transferred to the laptop.")
        ...
        while True:
            ...
            if distance == -1:
                ...
                if distance < DISTANCE_THRESHOLD:
                    ...
                    # Calculate price
                    try:
                        weight_grams = float(weight.split()[0])  # Extract numeric part
                        price = f"₹{(weight_grams / 100) * 5:.2f}"  # ₹5 per 100 grams
                    except ValueError:
                        price = "Invalid weight"

                    print(f"Price: {price}")

                    # Save and transfer the receipt
                    save_receipt_to_file(similarity, weight, price)
